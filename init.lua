...
transporter.cost_mult = minetest.settings:get("transporter_multiplier") or 5
    ...
    if enabled == 1 and eu_input >= demand then
        ...
        for k,v in pairs(objs) do
            if v:is_player() and v:get_player_name() ~= meta:get_string("last_user") then
				v:set_pos({x=dest.x + math.random(0,1)*2 - 1, y=dest.y + 1, z=dest.z + math.random(0,1)*2 - 1})
--				v:set_pos(dest)
        ...
        local objs_dest = minetest.get_objects_inside_radius({x=dest.x, y=dest.y, z=dest.z}, 0.8)

        transporter.effectsExit(dest)

        for k,v in pairs(objs_dest) do
            if v:is_player() and v:get_player_name() ~= meta:get_string("last_user") then
                v:set_pos({x=pos.x + math.random(0,1)*2 - 1, y=pos.y + 1, z=pos.z + math.random(0,1)*2 - 1})
...
minetest.register_node("transporter:transporter", {
    ...
    tiles = {"transporter_top.png", "transporter_bottom.png", "transporter_side.png", "transporter_side.png", "transporter_side.png", "transporter_front_off.png"},
